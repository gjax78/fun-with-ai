describe('Dashboard', () => {
  ...
  it('should display the AI engine\'s response once the form has been validated and the user submits', () => {
    ...
    cy.get('.response')
      .contains('1. She is passionate about building software that positively impacts our users and makes their complex problems simple. 2. She thrives in ambiguous situations. 3. She always has cute cat photos readily available.')
  })
  ...
  it('should clear the form once the form has been validated and the user submits', () => {
    cy.get('select')
      .select('Curie - Less capable, but fast')
    cy.get('.prompt-input')
      .type('Why should you consider Geena Jackson as your next Front End Software Engineer?')
    cy.get('.submit-button')
      .click()
    cy.get('.prompt-title')
      .contains('Prompt')
    cy.get('.response-title')
      .contains('Response')
    cy.get('.prompt')
      .contains('Why should you consider Geena Jackson as your next Front End Software Engineer?')
    cy.get('.response')
      .contains('1. She is passionate about building software that positively impacts our users and makes their complex problems simple. 2. She thrives in ambiguous situations. 3. She always has cute cat photos readily available.')
    cy.get('select')
